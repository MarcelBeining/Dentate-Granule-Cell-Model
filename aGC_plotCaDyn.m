...
for n = 1:numel(nneuron)
    load(t2n_expcat(targetfolder_data,'Exp_CaDyn',nneuron{n}.experiment),'plotcaivals','params','nodes','neuron','tree','CaNodes','mCai','stdCai','mV','stdV','tim','tw','maxcai')
...
%     set(c,'YTick',[-80,0,80])
    ...
    tprint(fullfile(targetfolder_results,t2n_expcat('CaMax-trees',nneuron{n}.experiment)),'-SHR-tif')
    ...
    tprint(fullfile(targetfolder_results,t2n_expcat('CaDecay',nneuron{n}.experiment)),'-pdf');
    ...
    FigureResizer(5,8,[barwidth,0.4])
    tprint(fullfile(targetfolder_results,t2n_expcat('CaAmp',nneuron{n}.experiment)),'-pdf');
