...
if any(options.show == [2 3])
    if exist(t2n_expcat(targetfolder_data,'Exp_VoltSteps',experiment),'file')
        load(t2n_expcat(targetfolder_data,'Exp_VoltSteps',experiment))
    ...
    if exist([t2n_expcat(targetfolder_data,'Exp_Spiking',experiment),'.mat'],'file')
        
        load([t2n_expcat(targetfolder_data,'Exp_Spiking',experiment),'.mat'])
...
FontResizer
% FigureResizer(5,8)
% tprint(fullfile(targetfolder_results,t2n_expcat('Fig.2-IV',neuron.experiment)),'-svg');
% tprint(fullfile(targetfolder_results,t2n_expcat('Fig.2-IV',neuron.experiment)),'-pdf');
